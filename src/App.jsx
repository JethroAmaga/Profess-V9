...
function GlobeScene({ radius, dots, markerColor, atmosphereColor, onSpun }) {
  ...
  return (
    <>
      ...
      <OrbitControls makeDefault enablePan={false} enableZoom={false} minDistance={5} maxDistance={15}
        ...
        onStart={() => { if (!firedRef.current) { firedRef.current = true; onSpun && onSpun(); } }}/>
    </>
  );
}

function Globe3D({ dots = [], radius = 2, markerColor = "#C8A870", atmosphereColor = "#C8A870", onSpun }) {
  return (
    <div style={{ position:"relative", height:"420px", width:"100%" }}>
      <Canvas gl={{ antialias:true, alpha:true, powerPreference:"high-performance" }} dpr={[1,2]}
        ...
        <Suspense fallback={null}>
          <GlobeScene radius={radius} dots={dots} markerColor={markerColor} atmosphereColor={atmosphereColor} onSpun={onSpun}/>
        </Suspense>
      </Canvas>
    </div>
  );
...
export default function Profess() {
  const [screen, setScreen] = useState("intro"); // intro | landing | lang | mode | disclaimer | intensity | scenario | session | summary
  ...
  useEffect(() => {
    ...
    const scrollEl = landingScrollRef.current;
    const globeEl = globeSectionRef.current;
    if (!scrollEl || !globeEl) return;
    globeEl.scrollIntoView({ behavior:"auto", block:"start" });
    const t = setTimeout(() => { scrollEl.scrollTo({ top:0, behavior:"smooth" }); }, 1000);
    ...
    return (
      <div className="screen-enter" style={{ ...BASE, height:"100vh", overflow:"hidden", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", position:"relative", padding:"24px", textAlign:"center" }}>
        ...
        <div style={{ width:"min(90vw,420px)" }}>
          <Globe3D dots={WORLD_MAP_DOTS} onSpun={() => { introJustFinished.current = true; setScreen("landing"); }}/>
        </div>
  ...
  const inputControls = (inputFs="15px", inputPad="12px 14px", btnSz="52px") => (
    <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
      ...
      <div style={{ display:"flex", gap:"8px", alignItems:"flex-end" }}>
        ...
        <div style={{ display:"flex", flexWrap:isMobile?"wrap":"nowrap", border:"1px solid #1A1A1A", width:isMobile?`calc(${btnSz} * 2)`:"auto", height:isMobile?`calc(${btnSz} * 2)`:btnSz, flexShrink:0 }}>
          <button onClick={askCoach} disabled={loading} title={lang==="id"?"Tanya Pelatih":"Ask Coach"}
            style={{ background:"none", border:"none", borderRight:"1px solid #1A1A1A", borderBottom:isMobile?"1px solid #1A1A1A":"none", color:"#C8A458", width:btnSz, height:btnSz, display:"flex", alignItems:"center", justifyContent:"center", cursor:loading?"default":"pointer", opacity:loading?0.4:1, transition:"background .2s, color .2s" }}
          ...
          <button onClick={toggleMic} className={isListening?"mic-active":""}
            style={{ background:isListening?"#180C0C":"none", border:"none", borderRight:isMobile?"none":"1px solid #1A1A1A", borderBottom:isMobile?"1px solid #1A1A1A":"none", color:isListening?"#BC7A7A":"#C8A458", width:btnSz, height:btnSz, display:"flex", alignItems:"center", justifyContent:"center", transition:"all .2s" }}
          ...
          <button id="psend" onClick={sendMessage} disabled={loading||!input.trim()}
            style={{ background:"none", border:"none", borderRight:"1px solid #1A1A1A", color:input.trim()&&!loading?sessionAccent:"#2A2A2A", width:btnSz, height:btnSz, display:"flex", alignItems:"center", justifyContent:"center", transition:"all .2s", opacity:loading||!input.trim()?0.3:1 }}
          ...
          <button onClick={endSession} title={lang==="id"?"Akhiri sesi":"End session"}
            style={{ background:"none", border:"none", borderRight:isMobile?"none":"none", color:"#C8A458", width:btnSz, height:btnSz, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"background .2s, color .2s" }}
