...
function GlobeScene({ radius, dots, markerColor, atmosphereColor }) {
  ...
  return (
    <>
      ...
      <OrbitControls makeDefault enablePan={false} enableZoom={false} minDistance={5} maxDistance={15}
        rotateSpeed={0.4} autoRotate autoRotateSpeed={0.3} enableDamping dampingFactor={0.1}/>
    </>
  );
}

function Globe3D({ dots = [], radius = 2, markerColor = "#C8A870", atmosphereColor = "#C8A870" }) {
  return (
    <div style={{ position:"relative", height:"420px", width:"100%" }}>
      <Canvas gl={{ antialias:true, alpha:true, powerPreference:"high-performance" }} dpr={[1,2]}
        ...
        <Suspense fallback={null}>
          <GlobeScene radius={radius} dots={dots} markerColor={markerColor} atmosphereColor={atmosphereColor}/>
        </Suspense>
      </Canvas>
    </div>
  );
...
export default function Profess() {
  ...
  if (screen === "landing") {
    ...
    const livingRoomMaleMobile = buildSVG(sofaLeft, agitated.roomMale ? "uncomfortable" : roomMood, false, "livingroom");
    const livingRoomFemaleMobile = buildSVG(sofaRight, agitated.roomFemale ? "uncomfortable" : roomMood, false, "livingroom");
        ...
        <div className="hero-section" style={{ position:"relative", height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden", paddingBottom:"0" }}>
          ...
          <div className="hero-living-room" style={{ position:"absolute", right:0, top:0, height:"100%", width:"45%", opacity: warmMode ? 0.55 : 0.35, transition:"opacity 1.2s ease", zIndex:1 }}>
            <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" style={{ width:"100%", height:"100%" }}>
              ...
              <line x1="80.5" y1="67.5" x2="87" y2="67.5" stroke="#2E2E36" strokeWidth="0.45" opacity="0.9"/>
              <line x1="80.5" y1="69" x2="87" y2="69" stroke="#2E2E36" strokeWidth="0.45" opacity="0.9"/>
              <line x1="80.5" y1="70.5" x2="87" y2="70.5" stroke="#2E2E36" strokeWidth="0.45" opacity="0.9"/>
              {/* Radio knobs — with rim shading */}
              <circle cx="89" cy="68.2" r="1" fill="#6A6A72" stroke="#3A3A42" strokeWidth="0.15"/>
              <circle cx="88.7" cy="67.9" r="0.3" fill="#8A8A92" opacity="0.8"/>
              <circle cx="89" cy="71" r="1" fill="#6A6A72" stroke="#3A3A42" strokeWidth="0.15"/>
              <circle cx="88.7" cy="70.7" r="0.3" fill="#8A8A92" opacity="0.8"/>
              {/* Radio antenna */}
              ...
              {/* Lamp — contact shadow */}
              <ellipse cx="50" cy="69" rx="5.5" ry="0.8" fill="#000" opacity="0.3"/>
              {/* Lamp shade */}
              <polygon points="44,59 56,59 53.5,51 46.5,51" fill="url(#lampShadeGrad)" stroke="#4A3020" strokeWidth="0.3"/>
              {/* Lamp shade — fold highlight */}
              <line x1="48.2" y1="58.3" x2="50.5" y2="51.6" stroke={warmMode ? "#F8D880" : "#5A4020"} strokeWidth="0.25" opacity="0.45"/>
              {/* Lamp base */}
              <rect x="47.5" y="59" width="5" height="9" rx="0.8" fill="url(#lampBaseGrad)"/>
              {/* Lamp foot */}
        ...
        <div className="hero-living-room-mobile" style={{ display:"none", width:"100%", height:"300px", overflow:"hidden", opacity: warmMode ? 0.7 : 0.55, transition:"opacity 1.2s ease" }}>
          <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet" style={{ width:"100%", height:"100%" }}>
            ...
            <foreignObject x="-4" y="32" width="48" height="54" overflow="visible" opacity={1}>
              <div xmlns="http://www.w3.org/1999/xhtml" style={{ width:"100%", height:"100%", perspective:"900px" }}>
                <div data-gyro-tilt="1" style={{ width:"100%", height:"100%", transformStyle:"preserve-3d", transition:"transform .2s ease-linear" }} onMouseMove={bodyTiltMove("roomMale")} onTouchMove={bodyTiltMove("roomMale")} onMouseLeave={bodyTiltLeave("roomMale")} onTouchEnd={bodyTiltLeave("roomMale")}>
                  <div style={{ width:"100%", height:"100%", animation:"friendBob 3.2s ease-in-out infinite" }} dangerouslySetInnerHTML={{ __html: livingRoomMaleMobile }}/>
                </div>
              </div>
            </foreignObject>
            <foreignObject x="56" y="32" width="46" height="52" overflow="visible" opacity={1}>
              <div xmlns="http://www.w3.org/1999/xhtml" style={{ width:"100%", height:"100%", perspective:"900px" }}>
                <div data-gyro-tilt="1" style={{ width:"100%", height:"100%", transformStyle:"preserve-3d", transition:"transform .2s ease-linear" }} onMouseMove={bodyTiltMove("roomFemale")} onTouchMove={bodyTiltMove("roomFemale")} onMouseLeave={bodyTiltLeave("roomFemale")} onTouchEnd={bodyTiltLeave("roomFemale")}>
                  <div style={{ width:"100%", height:"100%", animation:"friendBob 2.8s ease-in-out infinite", animationDelay:"0.4s" }} dangerouslySetInnerHTML={{ __html: livingRoomFemaleMobile }}/>
                </div>
              </div>
            </foreignObject>
            <ellipse cx="50" cy="79.5" rx="13" ry="1.2" fill="#000" opacity="0.25"/>
            ...
            <g onClick={() => { setWarmMode(w => { const next = !w; setRoomMood(next ? "warm" : "neutral"); return next; }); }} style={{ cursor:"pointer" }}>
              <polygon points="44,59 56,59 53.5,51 46.5,51" fill="url(#lampShadeGradMobile)" stroke="#4A3020" strokeWidth="0.3"/>
              <line x1="48.2" y1="58.3" x2="50.5" y2="51.6" stroke={warmMode ? "#F8D880" : "#5A4020"} strokeWidth="0.25" opacity="0.45"/>
              <rect x="47.5" y="59" width="5" height="9" rx="0.8" fill="url(#lampBaseGradMobile)"/>
              <rect x="45.5" y="67.5" width="9" height="1.5" rx="0.5" fill="#4F3A24"/>
...
>
  ...
  const charAnimBlock = (size, heightOverride) => {
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end" }}>
        <button onClick={askCoach} disabled={loading}
          style={{ background:"none", border:"1px solid #2A2520", color:"#C8A458", fontFamily:"inherit", fontSize:"10px", letterSpacing:".08em", textTransform:"uppercase", padding:"6px 12px", cursor:loading?"default":"pointer", opacity:loading?0.4:1, transition:"border-color .2s, color .2s", display:"flex", alignItems:"center", gap:"6px" }}
          onMouseEnter={e=>{ if(!loading){ e.currentTarget.style.borderColor="#C8A870"; e.currentTarget.style.color="#C8A870"; } }}
          onMouseLeave={e=>{ e.currentTarget.style.borderColor="#2A2520"; e.currentTarget.style.color="#C8A458"; }}>
          ◆ {lang==="id" ? "Tanya Pelatih" : "Ask Coach"}
        </button>
      </div>
  ...
  return (
    <div style={{ ...BASE, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:`0 ${isMobile?"16px":"32px"}`, height:isMobile?"48px":"56px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center", gap:"16px", flexShrink:0 }}>
        <SquigglyText style={{ fontFamily:"'Playfair Display',serif", fontSize:isMobile?"15px":"17px", fontWeight:500, letterSpacing:".06em" }}>Profess</SquigglyText>
        {!isMobile && <>
